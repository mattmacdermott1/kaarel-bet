...
def get_names(config: Dict[str, Any]) -> List[str]:
    ...
    """
    nd = NameDataset()
    names_dict = nd.get_top_names(n=config['dataset']['n_names']//2, country_alpha2='GB') # get_top_names returns the top n male and top n female, so we set n to n_names//2
...
def split_data(tuples: List[Tuple[str, str, str]], config: Dict[str, Any]) -> Tuple[List[Tuple[str, str, str]], List[Tuple[str, str, str]], List[Tuple[str, str, str]]]:
...
def save_jsonl(data: List[Tuple[str, str, str]], countries_capitals: List[Tuple[str, str]], output_path: str, mode: str):
...
def main():
    ...
    print(f"  Files saved:")
    print(f"    • data/dataset.csv (full dataset)")
    print(f"    • data/train.jsonl (training data)")
    print(f"    • data/val.jsonl (validation data)")
    print(f"    • data/test.jsonl (test data)")
    print("=" * 50)
